...
拾壹.服务异步通讯RabbitMQ的高级特性
    ...
    MQ集群：
        集群分类：
            RabbitMQ的底层是基于Erlang语言编写的。而Erlang语言又是一个面向并发的语言，天然至此集群模式，RabbitMQ的集群有两种模式：
                普通集群：
                    是一种分布式集群，将队列分散到队列的各个节点，从而提高整个集群的并发能力。
                    但是集群中的某个节点出现了故障，那这个节点上的队列和消息就全都没了，所以普通模式存在一定的安全性问题。
                镜像集群：
                    是一种主从集群，在普通集群的基础上，添加了主从备份的功能。提高集群的数据可用性。
                    镜像集群虽然至此主从，但主从同步并不是强一致的，某些情况下可能有数据丢失的风险。因此在RabbitMQ的3.8版本以后
                    推出了新的功能：仲裁队列来代替镜像集群，底层采用Raft协议确保主从的数据一致性。
        普通集群：
            普通集群也称为标准集群（classic cluster），具备以下特征：
        ...
        镜像集群：（这个章节可以跳过，直接看仲裁队列章节）
            镜像集群本质是主从模式，具备以下特征：
                ...
                创建队列的节点被称为该队列的主节点，备份到的其他的节点叫做该队列的镜像节点。
                一个队列的主节点还可能是另一个队列的镜像节点。
                    比如：节点1是队列1的主节点，同时节点1还是队列3的镜像节点。
